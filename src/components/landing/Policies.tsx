...
const POLICY_FEATURES = [
  {
    ...
    response: (
      <>
        ...
        <span className="bg-[#fef3c7] text-[#92400e] border border-[#fde68a] px-1.5 py-0.5 rounded font-mono text-[12px] font-semibold">
          &#123;NAME_1&#125;
        </span>{" "}
        at{" "}
        <span className="bg-emerald-100 text-[#1A5C38] border border-emerald-200 px-1.5 py-0.5 rounded font-mono text-[12px] font-semibold">
          &#123;EMAIL_1&#125;
        </span>
        .
      </>
...
export default function Policies() {
  ...
  return (
    <section className="py-32 px-6 bg-white" id="policies">
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div
          ...
          className="bg-gray-50 rounded-3xl overflow-hidden flex flex-col md:flex-row min-h-[560px]"
        >
          
          {/* Left Side: Forest Green Container with Dynamic Physical Card Shuffle Stack */}
          <div className="md:w-7/12 bg-[#1A5C38] p-8 md:p-12 flex flex-col items-center justify-center relative overflow-hidden">
            
            {/* Card Deck Viewport */}
            <div className="w-full max-w-md h-[430px] relative flex items-center justify-center pt-8">
              
              {POLICY_FEATURES.map((item, index) => {
                // Determine order in stack relative to activeFeature
                ...
                return (
                  <motion.div
                    ...
                    initial={false}
                    animate={{
                      y: order === 0 ? 0 : order === 1 ? -14 : order === 2 ? -26 : -36,
                    ...
                    className="absolute inset-x-0 bg-white rounded-3xl p-6 md:p-8 border border-gray-200 shadow-2xs flex flex-col justify-between h-[400px] pointer-events-auto"
                    style={{ originY: 0 }}
                  >
                    {/* Top Row - ChatGPT Icon */}
                    <div className="flex items-center gap-2">
                      <PlatformIcon platform="chatgpt" size={22} />
                    </div>

                    {/* Prompt Text */}
                    <p className="text-[14px] md:text-[15px] font-semibold text-gray-950 tracking-tight leading-snug font-sans">
                      {item.prompt}
                    </p>

                    {/* AI Protected Response Bubble */}
                    <div className="bg-[#f0f2f5] text-gray-900 rounded-2xl p-3.5 text-[13px] leading-relaxed font-sans max-w-[90%] ml-auto">
                      {item.response}
                    </div>

                    {/* Bottom ChatGPT Chat Input Bar */}
                    <div className="bg-[#f4f4f5] rounded-2xl p-3 flex flex-col justify-between h-[80px]">
                      <span className="text-[12px] text-gray-400 font-normal">Ask AI Assistant...</span>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-white border border-gray-200 flex items-center justify-center text-gray-400">
          ...
          <div className="md:w-5/12 bg-white divide-y divide-gray-100 flex flex-col justify-between border-l border-gray-100 relative">
            {POLICY_FEATURES.map((item, i) => {
              ...
              return (
                <div
                  ...
                  className="p-6 md:p-7 flex flex-col justify-center cursor-pointer flex-1 relative overflow-hidden group"
                >
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center gap-2.5 mb-1.5">
                      ...
                      />
                      <h4 className={`text-[14px] font-semibold tracking-tight transition-colors ${
